...
%type <ei> EOL
%type <ei> statement
%token INT FLOAT DOUBLE CHAR BOOL ARRAY_TYPE START INC DEC EOL FOB FCB FACTORIAL POWER VOID EXIT PI WHILE FOR IF ELSEIF ELSE THEN PRINT SCAN SWITCH CASE DEFAULT ASSIGN LOG10 LN EXP SQRT FLOOR CEIL ABS SIN ASIN COS ACOS TAN ATAN BREAK INCLUDE HEADER FUNC 
...
program:
	include functions START '{' lines '}'	{printf("\n Program executed successfully\n");}
	|
	;
functions:
		| function function
...
declaration :
	type id EOL	{printf("\nValid declaration\n");}
	| array_declaration EOL 	{printf("\nArray declaration Successful\n");}
	;
array_declaration :
		ARRAY_TYPE type VARIABLE '[' NUMBER ']'	{ 
									if(isdeclared($3)==1)
									{printf("\nDouble declaration!");}
									else
									{
...
statement	:
	...
	| switch_statements	{}
	| FUNC VARIABLE FOB FCB EOL	{
								
				                     printf("function (%s) is called",$2);
								}
...
expression	:	NUMBER	{$$ = $1;}
		...
		| expression '+' expression	{ 	int d=$3,c=$1,r;
									r = c + d; 
								printf("Sum=\n",r);
									$$=r;
							}
